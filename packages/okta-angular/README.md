...
Returns a promise that will resolve with the result of the OpenID Connect `/userinfo` endpoint if an access token is available in storage, or returns the claims of the ID token if no access token is available.  The returned claims depend on the requested response type, requested scopes, and authorization server policies. Error would be thrown if neither `/userinfo` endpoint nor the claims of the ID token can resolve user claims.  For more information see documentation for the [UserInfo endpoint][], [ID Token Claims][], and [Customizing Your Authorization Server][].
