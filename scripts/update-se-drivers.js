...
axios.get(chromeDriverUrl).then((response) => {
  ...
  console.log(`${pwd}/node_modules/webdriver-manager/bin/webdriver-manager update --versions.chrome ${chromeDriverVersion} --gecko false --versions.standalone latest`);
  execSync(`${pwd}/node_modules/webdriver-manager/bin/webdriver-manager update --versions.chrome ${chromeDriverVersion} --gecko false --versions.standalone latest`);
}).catch((err) => {
  console.log(err);
});
